...
LRT=2* (n*log(n)+naa*log(pmax(1,naa))+nab*log(pmax(1,nab))+nbb*log(pmax(1,nbb)) -(n/2)*log(n/2)-n1*log(n1)-n2*log(n2)-nab*log(2)) # n is # alleles = 2* n obs
...
png("SNPDepth.png",width = 480, height = 480)
 plot(snpdepth.non0 ~ snpdepth, col="#80808080",pch=16,cex=1.2,main="SNP Depth",ylab="Proportion of samples with SNP results",xlab="Mean SNP depth")
 dev.off()
}
...
qqplot(qchisq(ppoints(nsnps), df = 1), LRT , main = "Hardy-Weinberg LRT Q-Q Plot", xlab = parse(text="Theoretical ~~ (chi[1]^2) ~~  Quantiles"), ylab = "Sample Quantiles")
dev.off()
...
calcG <- function(snpsubset,sfx="",puse,indsubset,depth.min=0,depth.max=Inf,npc=0) {  # sfx is text to add to GGBS5 as graph name, puse is allele freqs (all snps) to use
 ...
 if (nsnpsub < nsnps) {  # this is before any depth filter ... needs correcting
 ...
 #depth0sub <- rowSums(depthsub==0)
 #snpdepthsub <- colMeans(depthsub)
 #snpdepthsub.non0 <- colSums(depthsub>0)/nrow(depthsub)
 ...
 if (!gform=="chip") {
  ...
  png(paste0("Gcompare",sfx,".png"),width = 960, height = 960 , pointsize=18)
   if (gform=="chip") plot(upper.vec(GGBS1) ~ upper.vec(GGBS5), col="#80808060",pch=16, main="Off-diagonal comparisons",xlab="Using G5", ylab="Using G1")
   if (!gform=="chip") pairs(cbind(upper.vec(GGBS1),upper.vec(GGBS3),upper.vec(GGBS5)), col="#80808060",pch=16, main="Off-diagonal comparisons",labels=paste0("Using G",c("1","3","5")))
   dev.off()
 if (npc>0) {
